...
/**
 * The main visitor for SpecSlice's first phase, which locates the target member(s) and compiles
 * information on what specifications they use.
 */
public class TargetMemberFinderVisitor extends SpecSliceStateVisitor {
  /**
   ...
   *
   * @param previous the previous SpecSlice visitor
  ...
  public TargetMemberFinderVisitor(
      SpecSliceStateVisitor previous, Map<String, String> nonPrimaryClassesToPrimaryClass) {
    super(previous);
  ...
  public Visitable visit(MethodDeclaration method, Void p) {
    ...
    if (this.targetMethodNames.contains(methodWithoutAnySpace)) {
      ...
      try {
        ...
        // be included in one of the classes that SpecSlice outputs.
      } catch (UnsolvedSymbolException e) {
