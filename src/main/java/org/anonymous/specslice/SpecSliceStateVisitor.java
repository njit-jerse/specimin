...
/**
 * This visitor contains shared logic and state for the SpecSlice's various XVisitor classes. It
 ...
 */
public abstract class SpecSliceStateVisitor extends ModifierVisitor<Void> {
   ...
   * assignments to these fields will be preserved, so SpecSlice needs to avoid adding an
   * initializer for them if they are final (as it does for other, non-assigned-by-target final
   * fields). Set by {@link TargetMemberFinderVisitor} but stored here so that it is easily
   * available later when pruning.
   */
   ...
   */
  public SpecSliceStateVisitor(SpecSliceStateVisitor previous) {
