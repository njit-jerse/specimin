...
/**
 * The main visitor for TypeSlice's first phase, which locates the target member(s) and compiles
 * information on what specifications they use.
 */
public class TargetMemberFinderVisitor extends TypeSliceStateVisitor {
  /**
   ...
   *
   * @param previous the previous TypeSlice visitor
  ...
  public TargetMemberFinderVisitor(
      TypeSliceStateVisitor previous, Map<String, String> nonPrimaryClassesToPrimaryClass) {
    super(previous);
  ...
  public Visitable visit(MethodDeclaration method, Void p) {
    ...
    if (this.targetMethodNames.contains(methodWithoutAnySpace)) {
      ...
      try {
        ...
        // be included in one of the classes that TypeSlice outputs.
      } catch (UnsolvedSymbolException e) {
