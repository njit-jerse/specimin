...
* ***--targetField***: a target field that must be preserved (including its initializer). Uses the same format as `--targetMethod`, but without the parameter list. At least one of `--targetMethod` or `--targetField` is required, but they can otherwise be freely combined.
